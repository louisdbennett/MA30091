...
glm2i = glm(current_smoker ~ age*marital_status + level_of_deprivation + rural + ethnicity + educ, 
            family = binomial(logit), data = dataR)

glm3i = glm(current_smoker ~ age + marital_status + level_of_deprivation*rural + ethnicity + educ, 
            family = binomial(logit), data = dataR)

glm4i = glm(current_smoker ~ age*marital_status + level_of_deprivation*rural + ethnicity + educ, 
            family = binomial(logit), data = dataR)
...
#Binning predicted probabilities for plotting
calibration_data$PredictedBin <- cut(calibration_data$Predicted, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE)

# Calculating mean actual outcome per bin
calibration_summary <- aggregate(Actual ~ PredictedBin, data = calibration_data, mean)

# Plotting
ggplot(calibration_summary, aes(x = as.numeric(as.character(PredictedBin)), y = Actual)) +
  geom_line() +
  geom_point() +
  labs(title = "Calibration Curve", x = "Mean Predicted Probability", y = "Mean Actual Outcome") +
...
model_formulas = list(
  glm1 = current_smoker ~ age + marital_status + level_of_deprivation + rural + ethnicity + educ,
  glm2 = current_smoker ~ age + marital_status + I(log(level_of_deprivation)) + rural + ethnicity + educ,
  glm3 = current_smoker ~ age * marital_status + level_of_deprivation + rural + ethnicity + educ,
  glm4 = current_smoker ~ age * marital_status + I(log(level_of_deprivation)) + rural + ethnicity + educ,
  glm5 = current_smoker ~ age * marital_status + I(log(level_of_deprivation))*rural + ethnicity + educ,
  glm6 = current_smoker ~ age * marital_status + level_of_deprivation*rural + ethnicity + educ
)
...
```{r}
#Think either model 1 or model 3 would be best; For the forest plot I will do model 1
...
glm_weekly1 = glm(nr_cigarettes_weekly ~ age + marital_status + level_of_deprivation + rural + ethnicity + educ, family = Gamma(), data = dataR_smokers)
