...
We start by investigating the data\ldots{}
...
weights to calculate a weighted MLE of \(p\). That is, letting \(w_i\)
denote the weight of the \(i^{th}\) observation, we alter the standard
likelihood to be
...
We also can see that this MLE has variance given by
\(var(\widehat{p})=\frac{p(1-p)}{\sum_{i=1}^{n}w_i}\), which we can
estimate using \(\widehat{p}\) and use the large sample Normal
approximation of the MLE to estimate our 95\% confidence intervals.
