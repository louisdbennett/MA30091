...
$$\logit(\mu_i) \sim \beta_0 + \beta_1a + \beta_2a^2 + \beta_3q + \alpha^{m}_j + \alpha^{u}_k + \alpha^{o}_l + \alpha^{t}_m + \alpha^{s}_n + \beta^{*}_kq$$
where $j \in \{Married,Not\,Married\}$, $k \in \{Urban,Not\,Urban\}$, $l \in \{White,...,Other\}$, $m \in \{Further\,Education,...,No\,qualification\}$ and $n \in \{Male,Female\}$
